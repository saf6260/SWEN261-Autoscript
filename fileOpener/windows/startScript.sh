...
#OS: WINDOWS
#Author Scott Frauenknecht
echo "Bash Script starting for file control. Please standby."

#Unzipping the .zip extension from current directory
echo "Unzipping file"
for file in *.zip
do
  if [ -e "$file" ] 
  then
    unzip -d webcheckers "$file" 
  else
    clear
...
start chrome http://localhost:4567/
start chrome --incognito http://localhost:4567/

#Opening the excel file for the testers
echo "opening excel file for use by testers"
cd etc/
start excel 'Acceptance Test Plan.xlsx'
cd ..
